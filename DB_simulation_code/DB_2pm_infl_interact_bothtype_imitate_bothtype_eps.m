function result = DB_2pm_infl_interact_bothtype_imitate_bothtype_eps(pm1, pm2, r, con_matrix, alpha, iter, N, p_ini, b)
    ...
    % Each user interacts with all their neighbors, but the payoff only depends on its own type.
    % They copy their strategy from the same type neighbour with (1-epislon), from different type with epsilon.
    % 2020.01.08
    % Yuejiang Li
    % ==========================
    % inputs:
    ...
    % ==========================
        ...
        % begin and end index for each type
        ...
        for i = 1:N
            ...
            sn_neigh_num = length(str_neigh) - sf_neigh_num;
            if action_table(i)
                if i <= type2_end
        ...
        while count <= iter
            for p = 1:N
                ...
                if i <= type2_end
                    ...
                    diff_type_friend = friend_list(friend_list > type2_end);
                else
                    same_type_friend = friend_list(friend_list > type2_end);
                    diff_type_friend = friend_list(friend_list <= type2_end);
                end
                fit_f = 0;
                fit_n = 0;
                sf_neigh_num = sum(action_table(friend_list));
                sn_neigh_num = friend_number - sf_neigh_num;
                ...
                % count strategy adoption of neighbors with different type
                str_diff_neigh = action_table(diff_type_friend);
                sf_diff_neigh_num = sum(str_diff_neigh);
                sn_diff_neigh_num = length(str_diff_neigh) - sf_diff_neigh_num;
                
                if action_table(i) == 1  % strategy update
                    if fit_f + fit_n == 0
                        trans_perc1 = 0;
                    else
                        trans_perc1 = fit_n/(fit_f + fit_n);
                    end
                    if sf_diff_neigh_num + sn_diff_neigh_num == 0
                        trans_perc2 = 0;
                    else
                        trans_perc2 = sn_diff_neigh_num / (sf_diff_neigh_num + sn_diff_neigh_num);
                    end
                    judge = (1 - epsilon) * trans_perc1 + epsilon * trans_perc2;
                    if rand <= judge
                        action_table(i) = 0;
                        % change fitness of the focal user and its neighbors
                        if i <= type2_end
                            fit_table(i) = fit_table(i) + (unf1 - uff1)*sf_neigh_num + (unn1 - ufn1)*sn_neigh_num;
                        else
                             fit_table(i) = fit_table(i) + (unf2 - uff2)*sf_neigh_num + (unn2 - ufn2)*sn_neigh_num;
                        end
                        for j = friend_list
                            if j <= type2_end
                                if action_table(j)
                    ...
                    if fit_f + fit_n == 0
                        ...
                        trans_perc1 = fit_f/(fit_f + fit_n);
                    end
                    if sf_diff_neigh_num + sn_diff_neigh_num == 0
                        trans_perc2 = 0;
                    else
                        trans_perc2 = sf_diff_neigh_num / (sf_diff_neigh_num + sn_diff_neigh_num);
                    end
                    judge = (1 - epsilon) * trans_perc1 + epsilon * trans_perc2;
                    if rand <= judge
                        action_table(i) = 1;
                        % change fitness of the focal user and its neighbors
                        if i <= type2_end
                            fit_table(i) = fit_table(i) + (uff1 - unf1)*sf_neigh_num + (ufn1 - unn1)*sn_neigh_num;
                        else
                            fit_table(i) = fit_table(i) + (uff2 - unf2)*sf_neigh_num + (ufn2 - unn2)*sn_neigh_num;
                        end
                        for j = friend_list
                            if j <= type2_end
                                if action_table(j)
